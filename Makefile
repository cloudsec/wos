...
KERNEL_OBJS = boot/load.o init/init.o init/version.o kernel/gdt.o kernel/trap_asm.o kernel/traps.o kernel/timer.o \
	kernel/sched.o kernel/system_call.o kernel/kthread.o kernel/fork.o kernel/exit.o kernel/sys.o kernel/panic.o \
	mm/memory.o mm/pfault.o mm/slab.o mm/buddy.o mm/page_fault.o fs/fs.o \
...
wos: $(KERNEL_OBJS)
	...
	#$(LD) -N -e _start -Ttext 0x7c00 -o boot.bin boot/boot.o
	#$(LD) -N -e startup_32 -Ttext 0x100000 -o kernel.bin $(KERNEL_OBJS) 
...
image:
	...
	objdump -d kernel.bin > kernel.hex
clean:
	cd boot/;make clean;
